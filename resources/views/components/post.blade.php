@props(["post" => $post])

<div class="mb-4 post" id="{{ $post->id }}">
    <a href="{{ route("users.posts", $post->user) }}" class="font-bold">{{ $post->user->name }}</a> <span
        class="text-gray-600 text-sm">{{ $post->created_at->diffForHumans() }}</span>
    <p class="mb-2">{{ $post->body }}</p>

    <div class="flex items-center">
        <span id="like-counter">{{ $post->likes->count() }}</span>
        <form id="like-form" action="" method="post" class="mr-1">
            @csrf
            <button id="like-button" type="submit"
                    class="text-blue-500">{{ $post->likedBy(auth()->user()) ? "Unlike" : "Like" }}</button>
        </form>
        @can("delete", $post)
            <form action="{{ route("posts.destroy", $post) }}" method="post">
                @csrf
                @method("DELETE")
                <button type="submit" class="text-red-700">Delete</button>
            </form>
        @endcan
    </div>
</div>
